...
本来这只是复旦大学《程序设计语言原理》的课程作业，目标是“实现简单的卡牌效果解析”。但写着写着，玩过的卡牌对战游戏点燃了我的中二之魂。我不再满足于简单的 `(damage 5)`，而是希望能够复刻一些更加神秘的卡牌机制，于是它变成了一个支持**变量绑定、条件分支、循环递归、副作用结算、甚至能把自己烧死**的**图灵完备**（存疑）的效果引擎。

正经的课程报告（包含 BNF 范式、架构设计、状态机模型），请移步：
...
- **强大的上下文控制：** 支持 `user`（使用者）、`intent`（意图）、`target`（实际目标）的动态切换。完美支持“对随机敌人施加基于自身当前烧伤层数一半的中毒”这种诡异特效。
...
- `effect.rkt`: 包含所有 `match` 逻辑和**效果分发与结算**。
...
- **没有抽牌堆/弃牌堆：** `env` 里虽然有卡牌列表，但没有实现洗牌和循环抽卡的逻辑（_因为和DSL解释关系不大_）
- **缺少事件触发器：** 无法实现“当受到攻击时”、“回合结束时”的自动触发效果（虽然硬编码了一些状态结算）。
